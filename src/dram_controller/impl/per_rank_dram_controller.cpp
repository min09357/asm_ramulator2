...
class PerRankDRAMController final : public IDRAMController, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAMController, PerRankDRAMController, "PerRank", "A DRAM controller with per-rank buffer count limits.");
  private:
    ...
    int m_num_ranks = -1;
    int m_max_read_per_rank = 0;
    int m_max_write_per_rank = 0;
    int m_base_read_buffer_size = 32;
    int m_base_write_buffer_size = 32;
    bool m_scale_buffer_by_ranks = true;
    std::vector<int> m_rank_read_count;
    std::vector<int> m_rank_write_count;
  ...
  public:
    void init() override {
      ...
      m_max_read_per_rank = param<int>("max_read_per_rank").desc("Max read buffer entries per rank (0=even split, <0=unlimited).").default_val(0);
      m_max_write_per_rank = param<int>("max_write_per_rank").desc("Max write buffer entries per rank (0=even split, <0=unlimited).").default_val(0);
      m_scale_buffer_by_ranks = param<bool>("scale_buffer_by_ranks").desc("Scale buffer size by number of ranks.").default_val(true);
      ...
      m_base_read_buffer_size = read_buffer_size;
      m_base_write_buffer_size = write_buffer_size;
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      // Optionally expand buffer size by number of ranks
      if (m_scale_buffer_by_ranks) {
        m_read_buffer.max_size = static_cast<size_t>(m_base_read_buffer_size * m_num_ranks);
        m_write_buffer.max_size = static_cast<size_t>(m_base_write_buffer_size * m_num_ranks);
      }

      // Per-rank count limit
      m_rank_read_count.resize(m_num_ranks, 0);
      m_rank_write_count.resize(m_num_ranks, 0);
      if (m_max_read_per_rank == 0)
        // m_max_read_per_rank = m_base_read_buffer_size;
        m_max_read_per_rank = m_read_buffer.max_size / m_num_ranks;
      if (m_max_write_per_rank == 0)
        // m_max_write_per_rank = m_base_write_buffer_size;
        m_max_write_per_rank = m_base_write_buffer_size / m_num_ranks;
    ...
    bool send(Request& req) override {
      ...
      if        (req.type_id == Request::Type::Read) {
        if (m_max_read_per_rank > 0 && m_rank_read_count[rank_id] >= m_max_read_per_rank) {
          req.arrive = -1;
          return false;
        }
        is_success = m_read_buffer.enqueue(req);
        if (is_success) m_rank_read_count[rank_id]++;
      } else if (req.type_id == Request::Type::Write) {
        if (m_max_write_per_rank > 0 && m_rank_write_count[rank_id] >= m_max_write_per_rank) {
          req.arrive = -1;
          return false;
        }
        is_success = m_write_buffer.enqueue(req);
        if (is_success) m_rank_write_count[rank_id]++;
      } else {
    ...
    void tick() override {
      ...
      if (request_found) {
        ...
        if (req_it->command == req_it->final_command) {
          ...
          // Decrement per-rank counter before removing from read/write buffer
          if (buffer == &m_read_buffer || buffer == &m_write_buffer) {
            int rank_id = req_it->addr_vec[m_rank_addr_idx];
            if (buffer == &m_read_buffer)  m_rank_read_count[rank_id]--;
            if (buffer == &m_write_buffer) m_rank_write_count[rank_id]--;
          }
