...
class PerRankRefresh : public IRefreshManager, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IRefreshManager, PerRankRefresh, "PerRank", "Per-Rank Refresh scheme.")
  private:
    Clk_t m_clk = 0;
    IDRAM* m_dram;
    IDRAMController* m_ctrl;
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      m_next_refresh_cycle = (m_nrefi/m_num_ranks);

      m_next_refresh_rank = 0;
    };
    ...
    void tick() {
      ...
      if (m_clk == m_next_refresh_cycle) {
        ...
        std::vector<int> addr_vec(m_dram_org_levels, -1);
        addr_vec[0] = m_ctrl->m_channel_id;
        addr_vec[1] = m_next_refresh_rank;
        Request req(addr_vec, m_ref_req_id);
        ...
        m_next_refresh_rank = (m_next_refresh_rank + 1) % m_num_ranks;
      }
    };
